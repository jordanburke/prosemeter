...
console.log(`n = ${scored.length}  (${short} below grade-band's 30-word floor)  ${((Date.now() - t0) / 1000).toFixed(1)}s`)
...
table("prosemeter", [
  ["median-of-five grade (internal)", "pmMedian"],
...
for (let i = 0; i < 10; i++) {
  ...
  console.log(`  decile ${String(i + 1).padStart(2)}: composite ${mean(chunk.map((c) => c.composite)).toFixed(1).padStart(5)}   BT_easiness ${mean(chunk.map((c) => c.bt)).toFixed(3).padStart(6)}`)
}

console.log("\nImplementation agreement against the corpus's independent computation")
for (const [label, a, b] of [["Flesch-Kincaid", "pmFk", "fk"], ["Flesch Reading Ease", "pmFre", "fre"]]) {
