...
- c3cbf77: `terminology-consistency` and `acronym-definition` no longer treat ALL-CAPS as a signal it is not
  ...
  **`terminology-consistency`** decided a term had "meaningful casing" via
  ...
  UPDATE/Update. A form now qualifies only when it has an internal capital _and_ is not entirely
