...
- 57a214a: `directness` findings now lead with checking the hedge rather than cutting it
...
- 84ddfc5: directness: stop flagging "May" the month as a hedge

  The upstream `hedges` list carries lowercase `may` and retext-intensify matches
  case-insensitively, so every date in a changelog or a timeline drew _"Cut the
  hedge or replace it with a concrete claim."_
...
- 8145374: directness: stop flagging technical nouns and plain verbs as hedges
